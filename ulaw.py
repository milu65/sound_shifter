...
def ulaw_clip_pcm_samples(pcm_samples, clip_value=8159):
    res = []
    for i in range(len(pcm_samples)):
        if abs(pcm_samples[i][0]) > clip_value:
            if pcm_samples[i][0] > 0:
                res.append(clip_value - 1)
            else:
                res.append(-clip_value + 1)
        else:
            res.append(pcm_samples[i][0])
    return res
...
def encode(pcm_code):
    ...
    for i in pcm_code:
        if i >= 0:
            encoded.append(ulaw_encode[int(i / 4)])
        else:
            encoded.append(0x7F & ulaw_encode[int(i / -4)])
    return np.asarray(encoded)


def decode(ulaw_code):
    decoded = []
    for i in ulaw_code:
        decoded.append(ulaw_decode[i])
