...
def pcm_to_adpcm(pcm_data):
    ...
    output_buffer = [0, 0]
    buffer_step = [True, True]
    adpcm_data = array.array('h')

    # ADPCM编码过程
    for i in range(0, len(pcm_data), 4):
        for channel in range(2):
            val = struct.unpack('<h', pcm_data[i + channel*2:i + (channel+1)*2])[0]
            diff = val
            sign = 0
            ...
            delta = 0
            vpdiff = stepsizeTable[output_buffer[channel] >> 3]
            ...
            if sign != 0:
                val = val - stepsizeTable[output_buffer[channel] >> 3]
            else:
                val = val + stepsizeTable[output_buffer[channel] >> 3]
            ...
            delta |= sign
            index = output_buffer[channel] + indexTable[delta]
            ...
            if buffer_step[channel]:
                output_buffer[channel] = delta & 0x0F
            else:
                adpcm_data.append(((delta << 4) & 0xF0) | output_buffer[channel])
                buffer_step[channel] = not buffer_step[channel]
...
def write_adpcm_to_aiff(original_aiff_path, adpcm_data):
    # 打开原始 AIFF 文件并读取其内容
    ...
    #print("read success")
    data_chunk_start = original_data.find(b'SSNR') + 4
    #print("find success")
    data_chunk_size = struct.unpack('>I', original_data[data_chunk_start:data_chunk_start + 4])[0]

    # 将 ADPCM 数据写回原始 AIFF 文件的 DATA 块中
    with open(original_aiff_path, 'wb') as modified_file:
        # 写回原始 AIFF 文件的头部
        modified_file.write(original_data[:data_chunk_start + 4])

        # 写入 ADPCM 数据的大小
        modified_file.write(struct.pack('>I', len(adpcm_data)))

        # 写入 ADPCM 数据
        modified_file.write(adpcm_data.tobytes())

        # 如果 DATA 块有额外的字节，将其写回
        if len(adpcm_data) < data_chunk_size:
            modified_file.write(original_data[data_chunk_start + 4 + len(adpcm_data):])
